...
param_cols <- c("resolution", "n_comp", "n_neig", "n_hvg")
...
metrics_corr <- metrics %>%
  ...
  summarize(
    rho = cor(param_value, metric_value, method = "spearman", use = "pairwise.complete.obs"),
    .groups = "drop"
  ) %>%
  mutate(
...
param_list <- c("resolution", "n_comp", "n_neig", "n_hvg")
...
sensitivity_df <- ari_long %>%
  ...
  mutate(
    ...
    parameter = factor(
      parameter,
      levels = c("resolution", "num components", "num neighbours", "num hvgs")
    )
  )
...
tradeoff_df <- metrics %>%
  filter(filtering == "manual") %>%
  select(
    dataset, method, resolution, filtering, n_comp, n_neig, n_hvg,
