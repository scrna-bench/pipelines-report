...
timings_renamed <- timings %>%
  rename_with(~ paste0("timings_", .x), all_of(timing_value_cols))

metrics_all <- bind_cols(metrics, select(timings_renamed, -all_of(keys_m)))
...
We only include runs with manual filtering, `n_comp = 30`, `n_neig = 20`, and `n_hvg = 1000`.
